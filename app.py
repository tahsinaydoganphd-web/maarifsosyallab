...
# ==========================================
# SEYRET BUL YÖNETİCİ İŞLEMLERİ (BURAYA EKLENDİ)
# ==========================================

@app.route('/api/seyret-bul/admin/get-all-videos', methods=['GET'])
def api_get_all_videos():
    """Admin paneli için tüm videoları listeler"""
    try:
        import seyret_bul # Modülü çağırdığımızdan emin olalım
        videos_dict = seyret_bul.videolari_yukle()
        
        videolar_listesi = []
        for vid_id, data in videos_dict.items():
            videolar_listesi.append({
                "video_id": vid_id,
        ...
        videolar_listesi.sort(key=lambda x: x['baslik'])
        return jsonify({"success": True, "videolar": videolar_listesi})
    except Exception as e:
        return jsonify({"success": False, "hata": str(e)})

@app.route('/api/seyret-bul/admin/delete-video', methods=['POST'])
def api_delete_seyret_bul_video():
    """Listeden seçilen videoyu siler"""
    try:
        import seyret_bul
        data = request.get_json()
        video_id = data.get('video_id')
        if not video_id: return jsonify({"success": False, "hata": "ID yok."})
            
        sonuc = seyret_bul.videoyu_sil(video_id)
        return jsonify(sonuc)
    except Exception as e:
        return jsonify({"success": False, "hata": str(e)})

@app.route('/api/seyret-bul/admin/edit-video', methods=['POST'])
def api_edit_seyret_bul_video():
    """Videoyu düzenler"""
    try:
        import seyret_bul
        import db_helper
        ...
        yeni_surec = data.get('yeni_surec')

        if not video_id: return jsonify({"success": False, "hata": "ID yok."})

        video = seyret_bul.video_detay_getir(video_id)
        if not video: return jsonify({"success": False, "hata": "Video yok."})

        if yeni_baslik: video['baslik'] = yeni_baslik
        if yeni_surec: video['surec_bileseni'] = yeni_surec
        
        # Veritabanına kaydet
        db_data = {
            "video_id": video['video_id'],
            "baslik": video['baslik'],
            "surec_bileseni": video['surec_bileseni'],
            "video_url": video.get('url'),
            "thumbnail_url": video.get('thumbnail_url'),
            "sure_saniye": video.get('sure_saniye'),
            "sorular_json": json.dumps(video.get('sorular', []), ensure_ascii=False)
        }
        
        if db_helper.save_video(db_data):
            return jsonify({"success": True, "mesaj": "Güncellendi."})
        else:
            return jsonify({"success": False, "hata": "DB Hatası."})
