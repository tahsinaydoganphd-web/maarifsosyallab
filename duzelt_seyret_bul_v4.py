...
# 1. APP.PY GÜNCELLEMESİ (Gemini JSON Hatasını Çöz)
app_path = 'app.py'
if os.path.exists(app_path):
    with open(app_path, 'r', encoding='utf-8') as f:
        app_content = f.read()

    # Daha sağlam bir değerlendirme fonksiyonu
    yeni_api_kodu = """
...
def api_seyret_bul_degerlendir():
    try:
        ...
        Yanıtın SADECE şu JSON formatında olsun, markdown kullanma: {{"skor": 3, "geri_bildirim": "..."}}'''
        ...
        try:
            ...
            # JSON bozuksa manuel puanla
            skor = 3
            geri_bildirim = f"Yapay zeka yanıtı işlenemedi: {text[:50]}..."
        
        return jsonify({"success": True, "skor": skor, "geri_bildirim": geri_bildirim})

    except Exception as e:
        print(f"Değerlendirme Hatası: {e}")
        return jsonify({"success": True, "skor": 1, "geri_bildirim": "Sistem hatası oluştu."})
"""
    
    # Eski fonksiyonu Regex ile bulup yenisiyle değiştiriyoruz
    # Regex, @app.route dekoratöründen başlayıp fonksiyon bitimine kadar olan bloğu hedefler
    pattern = r"@app\.route\('/api/seyret-bul/degerlendir'.*?def api_seyret_bul_degerlendir\(\):.*?return jsonify\(\{.*?\n\"\"\""
    # Ancak regex karmaşık olabileceği için, önceki eklediğimiz yerin tamamını replace edelim.
    # Eğer önceki kodunuz standartsa bu çalışır. Çalışmazsa 'append' mantığına döneriz.
    
    # Basit Replace yöntemi (Daha güvenli)
    if "def api_seyret_bul_degerlendir():" in app_content:
        # Mevcut içeriği temizleyip yenisini yazmak yerine, dosya sonuna ekleme veya 
        # kullanıcıdan manuel yapmasını isteme riskine girmeyelim.
        # Tüm dosyayı okuyup, ilgili fonksiyonu string operasyonuyla değiştirelim.
        
        # Başlangıç noktasını bul
        start_index = app_content.find("@app.route('/api/seyret-bul/degerlendir'")
        if start_index != -1:
            # Fonksiyonun bittiği yeri tahmin et (bir sonraki @app.route veya dosya sonu)
            end_index = app_content.find("@app.route", start_index + 1)
            if end_index == -1:
                # Dosya sonundaysa
                end_index = len(app_content)
                # if __name__ bloğu varsa oraya kadar
                main_index = app_content.find("if __name__", start_index)
                if main_index != -1:
                    end_index = main_index

            # Eski bloğu çıkar
            old_block = app_content[start_index:end_index]
            # Yeni bloğu yerleştir
            app_content = app_content.replace(old_block, yeni_api_kodu + "\n\n")
            
            with open(app_path, 'w', encoding='utf-8') as f:
                f.write(app_content)
            print("✅ app.py: Değerlendirme fonksiyonu güncellendi.")
    else:
        print("ℹ️ app.py içinde eski fonksiyon bulunamadı, eklenmedi.")


# 2. HTML GÜNCELLEMESİ (Nokta boyutu ve Zamanlama)
...
<body class="flex h-screen">
    ...
    <main class="ml-72 flex-1 p-6 md:p-8 overflow-y-auto no-bounce">
        ...
        <div id="videoContainer" class="hidden mt-6 bg-gray-50 p-6 rounded-lg shadow-inner max-w-5xl mx-auto relative">
            ...
            <div id="timeline" class="w-full max-w-2xl mx-auto bg-gray-200 rounded-full relative hidden mt-0" style="height: 2px;">
    ...
    <script>
        ...
        function setupMarkers() {
            ...
            sorular.forEach(s => {
                ...
                // GÜNCELLEME: w-2 h-2 (Küçük Nokta)
                m.className = 'absolute w-2 h-2 bg-red-600 rounded-full border border-white shadow';
                m.style.left = `${pos}%`; m.style.top = '-3px'; m.style.transform = 'translateX(-50%)';
        ...
        function checkTimeYT() {
            const intv = setInterval(() => {
                ...
                try {
                    ...
                    // GÜNCELLEME: Erken durma payı (-0.5) kaldırıldı. Tam zamanında duracak.
        ...
        function checkTimeHTML5() {
            ...
            const target = sorular[currentSoruIndex].duraklatma_saniyesi;
            // GÜNCELLEME: Erken durma payı kaldırıldı.
        ...
        function checkCevap(soru, cvp) {
            ...
            if(soru.tip === 'CoktanSecmeli') {
                ...
                fetch('/api/seyret-bul/degerlendir', {
                    ...
                    // GÜNCELLEME: Undefined kontrolü
                    let puan = d.skor; 
                    if(puan === undefined || puan === null) puan = 0;
                    finishFeedback(puan >= 3, `Puan: ${puan}/5. ${d.geri_bildirim || 'Geri bildirim yok.'}`);
                }).catch(e => finishFeedback(true, "Bağlantı hatası, kaydedildi."));
            }
        }
...
print("✅ Seyret Bul (V4) Güncellendi: Noktalar küçültüldü, zamanlama ve API hatası giderildi.")
