...
<body class="flex h-screen">
    ...
    <main class="ml-72 flex-1 p-6 md:p-8 overflow-y-auto no-bounce">
        ...
        <div id="videoContainer" class="hidden mt-6 bg-gray-50 p-6 rounded-lg shadow-inner max-w-5xl mx-auto relative">
            ...
            <div id="timeline" class="w-full max-w-2xl mx-auto mb-4 bg-gray-200 rounded-full relative hidden mt-4" style="height: 2px;">
    ...
    <script>
        ...
        function createYTPlayer() {
            ...
            try {
                player = new YT.Player('player', {
                    ...
                    playerVars: { 'playsinline': 1, 'rel': 0 }, // autoplay YOK
                    events: { 'onStateChange': onPlayerStateChange }
                });
                setupMarkers();
            } catch (e) { alert("YouTube oynatıcısı başlatılamadı."); }
        }
        ...
        function createHTML5Player() {
            ...
            setupMarkers();
            // HTML5'te de otomatik başlatma (play) YOK
        }
        ...
        function setupMarkers() {
            ...
            sorular.forEach(s => {
                ...
                // ORİJİNAL BOYUT (w-3 h-3) ve ORTALAMA AYARI
                m.className = 'absolute w-3 h-3 bg-red-500 rounded-full border-2 border-white shadow';
                m.style.left = `${pos}%`; 
                m.style.top = '-5px'; // 2px çizgiye 12px noktayı ortalamak için
                m.style.transform = 'translateX(-50%)';
                m.title = `Soru: ${s.duraklatma_saniyesi}s`;
        ...
        function checkCevap(soru, cvp) {
            ...
            if(soru.tip === 'CoktanSecmeli') {
                dogru = (cvp === soru.dogru_cevap); msg = dogru ? "Tebrikler, doğru!" : `Yanlış. Cevap: ${soru.dogru_cevap}`;
                finishFeedback(dogru, msg);
            } else if(soru.tip === 'BoslukDoldurma') {
                dogru = (cvp.trim().toLowerCase() === soru.dogru_cevap.trim().toLowerCase()); msg = dogru ? "Harika!" : `Yanlış. Cevap: ${soru.dogru_cevap}`;
                finishFeedback(dogru, msg);
            } else {
                ...
                }).catch(e => finishFeedback(true, "Kaydedildi."));
            }
        }
...
print("✅ Seyret Bul (V3) güncellendi: Orijinal ince tasarım ve manuel başlatma geri yüklendi.")
