...
# --- SABITLER ---
VIDEOS_FILE = "seyret_bul_videos.json"
...
def videolari_yukle():
    """Videos.json dosyasından videoları yükler"""
...
def videolari_kaydet(videos):
    """Videoları JSON dosyasına kaydeder"""
    try:
...
def videoyu_sil(video_id):
    """
    Bir videoyu JSON veritabanından ve (eğer lokalse) diskten siler.
    """
    videos_dict = videolari_yukle()
    
    if video_id not in videos_dict:
        return {"success": False, "hata": "Video ID bulunamadı."}
        
    try:
        video_data = videos_dict[video_id]
        video_url = video_data.get("url", "")
        
        # Adım 1: Lokal video dosyasını sil (eğer varsa)
        if video_url.startswith("/videolar/"):
            # URL'den dosya adını al (örn: /videolar/dosya.mp4 -> dosya.mp4)
            filename = video_url.split('/')[-1]
            file_path = os.path.join('videolar', filename)
            
            if os.path.exists(file_path):
                os.remove(file_path)
                print(f"Lokal video dosyası silindi: {file_path}")
            else:
                print(f"Lokal video dosyası bulunamadı (kayıt yine de silinecek): {file_path}")
        
        # Adım 2: JSON kaydını sil
        del videos_dict[video_id]
        
        # Adım 3: Güncel JSON'u kaydet
        if videolari_kaydet(videos_dict):
            return {"success": True, "mesaj": f"Video ({video_id}) başarıyla silindi."}
        else:
            return {"success": False, "hata": "JSON dosyası kaydedilemedi."}
