...
# Bu HTML; Yeni Arayüz (Konu Seçimi) + Eski Zeki Motor (1-1-1 Soru Kuralı) birleşimidir.
...
<body class="flex h-screen">
    ...
    <main class="ml-72 flex-1 p-6 md:p-8 overflow-y-auto no-bounce">
        ...
        <div id="videoContainer" class="hidden mt-6 bg-gray-50 p-6 rounded-lg shadow-inner max-w-5xl mx-auto relative">
            <button id="closeVideo" class="absolute top-2 right-2 text-red-500 hover:text-red-700 text-2xl"><i class="fa-solid fa-circle-xmark"></i></button>
            <div id="player" class="mb-1 w-full aspect-video bg-black rounded-lg overflow-hidden"></div>
            <div id="timeline" class="w-full mb-4 bg-gray-300 h-2 rounded-full relative hidden mt-4">
                <div id="progress" class="bg-indigo-500 h-2 rounded-full absolute left-0 top-0"></div>
                <div id="markers"></div>
            </div>
        </div>
        ...
        <div id="soruModal" class="hidden fixed inset-0 bg-black bg-opacity-80 z-50 flex items-center justify-center p-4">
            <div class="bg-white rounded-xl max-w-2xl w-full p-8 shadow-2xl">
                <h4 id="soruMetni" class="text-2xl font-bold mb-6 text-gray-800 border-b pb-4"></h4>
                <div id="cevaplar" class="space-y-4"></div>
            </div>
        </div>
    </main>
    ...
    <script>
        ...
        // --- OYUN MOTORU (1-1-1 KURALI GERİ YÜKLENDİ) ---
        function openVideoModal(videoId) {
            const studentNo = localStorage.getItem('loggedInUserNo');
            if (!studentNo) { alert("Öğrenci girişi bulunamadı."); return; }
            ...
            fetch(`/api/seyret-bul/video-detay/${videoId}`).then(r=>r.json()).then(data => {
                ...
                if(allQuestions.length === 0) { alert("Bu videoda soru yok."); return; }

                // 1-1-1 HAVUZ ALGORİTMASI
                ...
                let pool_CS = unseen.filter(q => q.tip === 'CoktanSecmeli');
                let pool_BD = unseen.filter(q => q.tip === 'BoslukDoldurma');
                let pool_KC = unseen.filter(q => q.tip === 'KisaCevap');
                let reset = false;
                ...
                if(sorular.length < 3) { alert("Yeterli soru tipi yok."); return; }

                // Soruları Kaydet
                const newIds = sorular.map(q => q.id);
                if(reset) seenList = newIds; else newIds.forEach(id => { if(!seenList.includes(id)) seenList.push(id); });
        ...
        function loadYouTubeAPI() {
            if (!window.YT) { const t = document.createElement('script'); t.src = "https://www.youtube.com/iframe_api"; document.head.appendChild(t); window.onYouTubeIframeAPIReady = createPlayer; } else { createPlayer(); }
        }

        function createPlayer() {
            const pDiv = document.getElementById('player'); pDiv.innerHTML = '';
            ...
            sorular.forEach(s => {
                ...
                const pos = (s.duraklatma_saniyesi / videoData.sure_saniye) * 100;
                m.className = 'absolute w-4 h-4 bg-red-600 rounded-full border-2 border-white shadow';
                m.style.left = `${pos}%`; m.style.top = '-4px'; m.style.transform = 'translateX(-50%)';
                markers.appendChild(m);
            });

            if (videoData.url.includes('youtube') || videoData.url.includes('youtu.be')) {
                ...
                pDiv.innerHTML = `<video id="html5-player" controls style="width:100%;height:100%"><source src="${videoData.url}" type="video/mp4"></video>`;
        ...
        function checkTimeYT() {
            const intv = setInterval(() => {
                ...
                if(t >= target - 0.5 && t < target + 2) { player.pauseVideo(); showSoru(sorular[currentSoruIndex]); clearInterval(intv); }
            }, 200);
        }
        ...
        function checkTimeHTML5() {
            ...
            const target = sorular[currentSoruIndex].duraklatma_saniyesi;
            if(t >= target - 0.5 && t < target + 2) { player.pause(); showSoru(sorular[currentSoruIndex]); }
        }
        ...
        function checkCevap(soru, cvp) {
            ...
            // Basit Kontrol Mantığı (Detaylı Gemini kontrolü API'ye taşınabilir, şimdilik yerel kontrol)
            let dogru = false;
            let msg = "";
            ...
            if(soru.tip === 'CoktanSecmeli') {
                ...
                // Kısa cevap için sunucuya gitmek gerekir, şimdilik geçici mesaj
                // Gerçek entegrasyonda /api/seyret-bul/degerlendir çağrılmalı
                // Şimdilik basit onay veriyoruz
                fetch('/api/seyret-bul/degerlendir', {
                    method:'POST', headers:{'Content-Type':'application/json'},
                    body: JSON.stringify({soru_metni: soru.soru, kullanici_cevabi: cvp})
                }).then(r=>r.json()).then(d => {
                    finishFeedback(d.skor >= 3, `Puan: ${d.skor}/5. ${d.geri_bildirim}`);
                }).catch(e => finishFeedback(true, "Cevabınız kaydedildi."));
                return;
            }
        ...
        function finishFeedback(success, text) {
            ...
            div.innerHTML = `<h3 class="text-xl font-bold ${color} mb-4 text-center">${success ? 'DOĞRU' : 'YANLIŞ'}</h3>
        ...
        function resumeVideo() {
            ...
            if(currentSoruIndex < sorular.length) {
                if(player.playVideo) checkTimeYT(); // Re-attach interval
            }
        }
    </script>
</body>
...
print("✅ Seyret Bul sayfası tamir edildi: Arayüz + Zeki Soru Motoru birleştirildi.")
